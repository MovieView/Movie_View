...
export async function executeQury(userId: string, username: string, filePath: string) {
  const connection = await dbConnectionPoolAsync.getConnection();
  
  try {
    await connection.execute('SET SQL_SAFE_UPDATES=0');

    let updateQuery = 'UPDATE movie_view.social_accounts SET extra_data = \'{\"username\":\"';
