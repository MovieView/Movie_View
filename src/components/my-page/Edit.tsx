  ...
  return (
    <div className='bg-gray-100 w-2/5 h-auto text-black-600 rounded-lg mb-20 pb-10 border border-first'>
      <form onSubmit={handleSubmit} className='flex flex-col justify-center items-center space-y-4'>
        <div className='p-10 text-[20px] font-bold'>
          프로필 수정
          <FaHeart className='relative bottom-6 left-80 text-first' onClick={heartList} />
        </div>
        <div className='relative flex justify-center items-center w-32 h-32 rounded-full border-2 border border-gray-400 cursor-pointer'>
          ...
          />
          {preview ? (
            <Image src={preview} alt='Preview' layout="fill" objectFit="cover" className='rounded-full' />
          ) : (
            <Image src={session.user?.image || '/default-profile.png'} alt='Profile' layout="fill" objectFit="cover" className='rounded-full' />
          )}
        </div>
        <input
          type='text'
